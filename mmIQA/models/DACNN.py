...
class MSFB(BaseModule):
    def __init__(self, inchannel, outchannel):
        super(MSFB, self).__init__()

        self.conv1_MSFB = Conv2d(in_channels=inchannel, out_channels=outchannel, kernel_size=1, stride=2, padding=0)
        self.conv3_MSFB_s1 =Conv2d(in_channels=outchannel, out_channels=outchannel, kernel_size=3, stride=1, padding=1)
        self.conv3_MSFB_s2_1 = Conv2d(in_channels=inchannel, out_channels=outchannel, kernel_size=3, stride=2, padding=1)
        self.conv3_MSFB_s2_2 = Conv2d(in_channels=inchannel, out_channels=outchannel, kernel_size=3, stride=2, padding=1)
...
class WGB(BaseModule):
    def __init__(self, inchannel, outchannel):
        super(WGB, self).__init__()

        self.conv3_s1_1 = Conv2d(in_channels=inchannel, out_channels=outchannel, kernel_size=3, stride=1, padding=1)
        self.conv3_s1_2 = Conv2d(in_channels=inchannel, out_channels=outchannel, kernel_size=3, stride=1, padding=1)
...
class DACNN(BaseModel):
    def __init__(self):
        ...
        # TODO: introduce the pretrained
        self.ADANet = EfficientNet(arch='b0')
        
        # self.ADANet = EfficientNet.from_pretrained('efficientnet-b0')
        self.ADANet.eval()

        self.SDANet = EfficientNet(arch='b0', 
                                   init_cfg=[
                                       dict(type='Pretrained', 
                                            checkpoint='pretrain_model/model/EffnetPretraining.pth'
                                            )])        
    ...
    def forward(self,input):
        ...
        # a4 = endpoints_a['reduction_6']#[1, 1280, 7, 7]

        (a1, a2, a3, _, a4) = self.ADANet(input)

        #Synthetic Distortion Features Extraction
        #s = synthetic distortion
        (s1, s2, s3, _, s4) = self.SDANet(input)
